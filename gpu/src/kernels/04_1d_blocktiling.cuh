                                        ...
                                        const float *B, float *C)
{
    ...
    // // Blockdim is 1D (only x). Translate index between 0-1023 to index in (32, 32)
    // block const uint threadRow = threadIdx.x / BN; const uint threadCol = threadIdx.x
    // % BN;
    ...
    //     // Perform dot product over row vector of block in A and col vector of block
    //     in B for (int dotIdx = 0; dotIdx < BK; ++dotIdx)
    //     {
    //         for (int resIdx = 0; resIdx < TM; ++resIdx)
    //         {
    //         }
    //     }
    //     tmp += As[threadRow * BLOCKSIZE + dotIdx] * Bs[dotIdx * BLOCKSIZE +
    //     threadCol];

    //     // Need to synch again at the end to avoid faster threads fetching the next
    //     block
